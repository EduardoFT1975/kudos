...
// Phase P0.6 · beta-safe queue economics. Cap total probes at 6
// (2 rings × 3 bearings) with target 3 items. Worst-case per-session
// fetch budget: primary(1) + quality_retry(1) + probes(≤6) = 8, holds
// under backend rate limit 10/min/IP with comfortable headroom for
// user-initiated retries. Pre-patch was 15 max probes (3 rings × 5).
const _DEFAULT_TARGET = 3;
const _DEFAULT_PROBE_RADIUS = 800;
const _DEFAULT_RING_RADIUS = 1200;
const _RING_MULTIPLIERS: readonly number[] = [1, 2];
const _PROBE_BEARINGS_DEG: readonly number[] = [0, 144, 216];
// Idle gap between sequential probes. 400ms keeps main thread breathing
// AND lets primary capsule render fully before queue work competes for
// browser connection slots.
const _PROBE_IDLE_MS = 400;
    ...
    const drive = async () => {
      while (
        ...
        await new Promise<void>((resolve) =>
          window.setTimeout(resolve, _PROBE_IDLE_MS),
        );
      }
      if (
        !cancelled &&
