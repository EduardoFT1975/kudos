...
export function WorldEngine() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      // T7.3 · expose map + Leaflet globalmente para WorldUniversalLayer
      (window as any)._kudosWorldMap = map;
      (window as any).L = L;
    ...
    return () => {
      ...
      // T6.4.B · invalidar refs globales para que WorldUniversalLayer no
      // intente operar sobre un map destruido al remontar.
      try {
        if ((window as any)._kudosWorldMap === createdMap) {
          (window as any)._kudosWorldMap = null;
        }
        (window as any)._kudosUserPoiLayer = null;
      } catch {}
...
function ensureCss() {
  ...
  // T6.4.C · siempre reemplazar el style block para que cambios en
  // world-tokens.ts surtan efecto sin necesidad de cerrar la pestana.
  {
    const existingStyle = document.getElementById("kudos-world-style");
